...
const EditDog = () => {
  ...
  useEffect(() => {
    ...
    if (currentDog.imageUrl) {
      setImagePreview(currentDog.imageUrl);
    }
  }, [currentDog, navigate]);
  ...
  const defaultValues: Partial<DogFormValues> = {
    ...
    fullName: currentDog?.fullName || "",
    registrationNumber: currentDog?.registrationNumber || "",
    chipNumber: currentDog?.chipNumber || "",
    pedigree: currentDog?.pedigree || "",
    geneticTestResults: currentDog?.geneticTestResults || "",
    inbreedingCoefficient: currentDog?.inbreedingCoefficient || "",
    healthStatus: currentDog?.healthStatus || "",
    vaccinationHistory: currentDog?.vaccinationHistory || "",
    weight: currentDog?.weight || "",
    size: currentDog?.size || "",
    breedingStatus: currentDog?.breedingStatus || "",
    cycleInformation: currentDog?.cycleInformation || "",
    breedingHistory: currentDog?.breedingHistory || "",
    litterInformation: currentDog?.litterInformation || "",
    breedingRestrictions: currentDog?.breedingRestrictions || "",
    achievements: currentDog?.achievements?.[0] || "",
    exhibitionResults: currentDog?.exhibitionResults || "",
    temperamentAssessment: currentDog?.temperamentAssessment || "",
    notes: currentDog?.notes || "",
    imageUrl: currentDog?.imageUrl || "",
    hasStandardPhotos: currentDog?.hasStandardPhotos || false,
  };
  ...
  // Effect to set form values when currentDog changes
  useEffect(() => {
    if (currentDog) {
      Object.entries(defaultValues).forEach(([key, value]) => {
        form.setValue(key as keyof DogFormValues, value as any);
      });
    }
  }, [currentDog, form]);
  ...
  function onSubmit(data: DogFormValues) {
    ...
    updateDog({
      ...
      geneticTestResults: data.geneticTestResults,
      ...
      cycleInformation: data.cycleInformation,
      breedingHistory: data.breedingHistory,
      litterInformation: data.litterInformation,
      breedingRestrictions: data.breedingRestrictions,
      achievements: data.achievements ? [data.achievements] : [],
      exhibitionResults: data.exhibitionResults,
      temperamentAssessment: data.temperamentAssessment,
      notes: data.notes,
    ...
    toast.success("Hund erfolgreich aktualisiert!");
    setTimeout(() => navigate(`/dogs/${id}`), 1500);
  }

  const handleCancel = () => {
    if (form.formState.isDirty) {
      if (window.confirm("Möchten Sie wirklich abbrechen? Alle Änderungen gehen verloren.")) {
        navigate(`/dogs/${id}`);
      }
    } else {
      navigate(`/dogs/${id}`);
      ...
      <main className="flex-1 overflow-hidden pb-28">
        <ScrollArea className="h-[calc(100vh-64px)]">
          <div className="app-container">
            ...
            <Form {...form}>
              <form id="edit-dog-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
